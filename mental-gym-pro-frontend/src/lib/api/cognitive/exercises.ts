...
const MOCK_EXERCISES: Exercise[] = [
  {
    ...
    description:
      "Encuentra todas las parejas de cartas iguales lo más rápido posible.",
    category: "memoria",
    difficulty: "easy",
    duration: 5,
    instructions: [
  ...
  },
  {
    ...
    description:
      "Cambia entre diferentes reglas de clasificación lo más rápido posible.",
...
export async function fetchExercises(
  ...
  Exercise[] | { items: Exercise[]; total: number; page: number; limit: number }
> {
  if (USE_MOCK) {
  ...
  // En lugar de getJSON<any>, especifica los tipos de respuesta posibles.
  try {
    const res = await getJSON<
      | Exercise[]
      | { items: Exercise[]; total?: number; page?: number; limit?: number }
    >(paths);

    // El resto del código ya está preparado para manejar ambos formatos
  ...
  export async function startExerciseSession(
    ...
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        "[startExerciseSession] backend falló, sesión mock:",
        lastErr
      );
      return { _id: `sess_${Math.random().toString(36).slice(2, 11)}` };
    }
    throw lastErr instanceof Error
      ? lastErr
      : new Error(String(lastErr ?? "Cannot start session"));
  }
